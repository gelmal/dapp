...
function Usage() {
    console.log("Usage --> ");
    console.log("    node TestMainAddPollCrowdSaleAddress.js [argv1] [argv2] [argv3] [argv4]");
    console.log("....argv1: Owner 계정 (Index)");
    console.log("....argv2: Owner 암호");
    console.log("....argv3: 메인 컨트랙트 주소");
    console.log("....argv4: 등록할 추가크라우드세일 투표 주소");
}
...
let RunProc = async () => {
    try {
        ...
        console.log('* Param04 (CrpPollAdSale Address):....' + process.argv[5]);
        ...
        const polladsaleaddr = process.argv[5]; // 크라우드세일 컨트랙트 주소
        console.log('Owner Account: [' + owner + ']');
        if(await act.tryActions(api_main.addPollCrowdSaleAddress, showProgress, false, 5, true, owner, passwd, mainaddr, polladsaleaddr) == false) {
            throw new Error("Failed! (addPollCrowdSaleAddress)");
        }
        let mainobj = await api_main.getObject(mainaddr);
        let idx = mainobj.getPollSaleContractNum() - 1;
        console.log("### 등록된 추가세일 투표 인덱스 ==> [" + idx + "]");
        console.log("    등록된 추가세일 투표 주소 ==> [" + mainobj.crowd_poll_addrs(idx) + "]");
    } catch(err) {
        console.log(err);
    }
}
