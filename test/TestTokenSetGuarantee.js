...
function Usage() {
    ...
    console.log("....argv4: 추가 토큰을 받을 계정");
    console.log("....argv5: 추가 토큰 양");
}
...
let RunProc = async () => {
    try {
        ...
        console.log('* Param04 (Set Guarantee Staff):......' + process.argv[5]);
        console.log('* Param05 (Guarantee Amount):.........' + process.argv[6]);
        ...
        console.log("### [" + to + "]의 Guarantee 발란스: --> ", utils.getEther(tokenobj.guranteeOf(to).toString(10)));
        console.log("    총 통화량: --> ", utils.getEther(tokenobj.totalSupply().toString(10)));
        if(await act.tryActions(api_token.setGuarantee, showProgress, false, 6, true, owner, passwd, tokenaddr, to, utils.getWei(amount)) == false) {
            throw new Error("Failed! (setGuarantee)");
        }
        console.log("### [" + to + "]의 Guarantee 발란스: --> ", utils.getEther(tokenobj.guaranteeOf(to).toString(10), "done!"));
    } catch(err) {
        console.log(err);
    }
}
