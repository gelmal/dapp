...
let input_params = fs.readFileSync(args_dir + '/scn9_JoinPollWithdraw.json', 'utf8');
...
let abi_path = path.resolve(__dirname, 'abi', 'CrpPollWith.abi'); // abi가 저장된 file path
let abi = fs.readFileSync(abi_path, 'utf-8'); // abi 추출
let road = web3.eth.contract(JSON.parse(abi)); // get contract
let withdraw_contract = road.at(withdraw_address);
...
/**
 * withdraw poll을 실행하는 함수이다.
 * 1. unlock Account 수행
 * 2. 표 분류
 * 3. withdraw contract의 polling
...
let pollRoad = async () => {
    try {
        ...
        console.log("poll of withdraw by holder ....."); // 투표
        gas = await withdraw_contract.polling.estimateGas(boolean, {
            from: voter
        });
        let tx = await withdraw_contract.polling(boolean, {
            from: voter,
            gas: gas
        });
