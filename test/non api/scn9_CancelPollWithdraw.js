...
let input_params = fs.readFileSync(args_dir + '/scn9_CancelPollWithdraw.json', 'utf8');
let input = JSON.parse(input_params);
const provider = input.provider;
const voter = input.voter;
const passwd = input.passwd;
const withdraw_address = input.address;
...
let abi_path = path.resolve(__dirname, 'abi', 'CrpPoll.abi'); // abi가 저장된 file path
let abi = fs.readFileSync(abi_path, 'utf-8'); // abi 추출
let With = web3.eth.contract(JSON.parse(abi)); // get contract
let withdraw_contract = With.at(withdraw_address);
...
/**
 * withdraw에 대한 투표를 취소하는 기능
 * 1. unlock Account 수행
 * 2. withdraw의 cancelPoll 실행 
...
let CancelpollRoad = async () => {
    try {
        ...
        console.log("cancel poll of withdraw by holder .....");

        gas = await withdraw_contract.cancelPoll.estimateGas({
            from: voter
        });
        
        let tx = await withdraw_contract.cancelPoll({
            from: voter,
            gas: gas
        }); // 투표 취소
...
let RunProc = async () => {
    try {
        ...
        console.log('* param5 (withdraw contract):.' + input.address);
