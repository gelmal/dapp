...
let input_params = fs.readFileSync(args_dir + '/scn16_JoinPollRefund.json', 'utf8');
...
/**
 * refund poll을 실행하는 함수이다.
 * 1. unlock Account 수행
 * 2. 표 분류
 * 3. refund poll contract의 polling
...
let pollRoad = async () => {
    try {
        ...
        console.log("poll of refund holder ....."); // 투표
        gas = await refund_contract.polling.estimateGas(boolean, {
            from: voter
        });
        let tx = await refund_contract.polling(boolean, {
            from: voter,
            gas: gas
        });
