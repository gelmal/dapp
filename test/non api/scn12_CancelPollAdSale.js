...
let input_params = fs.readFileSync(args_dir + '/scn12_CancelPollAdSale.json', 'utf8');
...
let addsale_contract = With.at(addsale_address);
...
/**
 * addsale에 대한 투표를 취소하는 기능
 * 1. unlock Account 수행
 * 2. addsale의 cancelPoll 실행 
...
let CancelpollRoad = async () => {
    try {
        ...
        console.log("cancel poll of addsale by holder .....");

        gas = await addsale_contract.cancelPoll.estimateGas({
            from: voter
        });
        
        let tx = await addsale_contract.cancelPoll({
            from: voter,
            gas: gas
        }); // 투표 취소
